...
const adminController = {
    ...
    processNoShows: async (req, res) => {
        try {
            ...
            const noShows = await MealConfirmation.getNoShowUsers(date, meal_type);
            const mealTiming = await MealTiming.getByMealType(meal_type);
            
            if (!mealTiming) {
                return res.status(404).json({
                    error: 'Meal Timing Not Found',
                    details: 'Could not find meal timing configuration'
                });
            }
            ...
            res.status(500).json({
                error: 'No-Show Processing Failed',
                details: error.message
            });
        }
    },
