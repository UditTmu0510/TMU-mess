...
class MealTiming {
    constructor(data) {
        this._id = data._id; // meal type as ID
    ...
    static async getByMealType(mealType) {
        const db = getDB();
        return await db.collection('meal_timings').findOne({ _id: mealType, is_active: true });
    }
    ...
    static async getActiveMealTypesWithCosts() {
        ...
        const mealTimings = await db.collection('meal_timings')
            .find({ is_active: true })
            .project({ _id: 1, per_meal_cost: 1, start_time: 1, end_time: 1 })
            .toArray();
